...
test('clear()', (t: any) => {
	...
	keys.forEach((k: string) => foo.set(k, parseInt(k.charCodeAt(0).toString())));
...
test('ordering', (t: any) => {
	const foo = flru(3);
	const keys = (arr: string[], bool: boolean) => t.is(arr.every(foo.has), bool, `[${arr.toString()}] ~> ${bool ? '' : 'un'}known`);

	// A=[a,b,c]  &  S=[]
	['a', 'b', 'c'].forEach((k: string) => foo.set(k, parseInt(k.charCodeAt(0).toString())));
	keys(['a', 'b', 'c'], true);
